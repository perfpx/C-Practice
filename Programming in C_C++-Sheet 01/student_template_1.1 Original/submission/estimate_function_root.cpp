...
{
    float absolutDifference = std::abs(x1 - x2);

    if (absolutDifference <= epsilon) {
        return true;
    }
    return false;
}
...
float estimateFunctionRoot(float (*linearFunc)(float), float xLower, float xUpper, unsigned int nDecimals)
{
    ...
    float tempUp = xUpper;
    float tempLow = xLower;
    float tempMid = (xLower + xUpper) / 2;


    while(xValueFound == false) {
       
       if(linearFunc(tempLow) == 0) {
        returnValue = tempLow;
        xValueFound = true;
       }

       if(linearFunc(tempUp) == 0) {
        returnValue = tempUp;
        xValueFound = true;
       }

       if(linearFunc( tempMid ) == 0) {
        returnValue = tempMid;
        xValueFound = true;
       }

       if(linearFunc(tempUp) * linearFunc(tempMid) > 0) {
            tempUp = tempMid;
            tempMid = (tempLow + tempUp) / 2;

       } else if(linearFunc(tempLow) * linearFunc(tempMid) > 0){
            tempLow = tempMid;
            tempMid = (tempLow + tempUp) / 2;
       }

       if(isAlmostEqual(tempLow, tempUp)) {
        
         returnValue = linearFunc(tempLow) * linearFunc(tempUp) > 0 ? NAN : roundValToNDecimals(tempLow, nDecimals);
         xValueFound = true;
...
{ 
    ...
    std::cout << "Is x1 = 0.1 almost equal to x2 = 0.11 with 1 decimal precision?: " << isAlmostEqual(0.1f, 0.11f, 0.1f) << ", expected: " << "1" << std::endl << std::endl;
    std::cout << "Is x1 = 0.1 almost equal to x2 = 0.11 with 2 decimal precision?: " << isAlmostEqual(0.1f, 0.11f, 1e-2f) << ", expected: " << "1" << std::endl;
    std::cout << "Is x1 = 0.1 almost equal to x2 = 0.11 with 3 decimal precision?: " << isAlmostEqual(0.1f, 0.11f, 1e-3f) << ", expected: " << "0" << std::endl;
    ...
    testAndPrintDecreasing(-0.2f, 0.234f);

    // Test 1f) Special Case returning NAN
    testAndPrintDecreasing(-2.0f, -0.10f);
    testAndPrintDecreasing(-0.023f, 1.0f);
