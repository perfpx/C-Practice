...
public:
    ...
    public:
        ...
        // Function calculates distance between from and other
        float distanceFromTo(const ShortestPaths::Location& other) const;
...
public:
    ...
    std::vector<size_t> compute_shortest_path(size_t from, size_t to) const;
     ...
     // Function calculates the point with the shortest distance from starting point
        size_t shortestPoint(const std::vector<size_t> &vector, const std::vector<float> &distanceVec) const;
