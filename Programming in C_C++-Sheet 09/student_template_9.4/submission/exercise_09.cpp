...
public:
  ...
  stop() { // get time in ms and divide by 1000 ( to get 3-digit precision)
    t_sec = std::chrono::duration_cast<std::chrono::milliseconds>(
...
void print_hist(const vector<int> &h) {
  for (size_t i = 0; i != h.size(); ++i) {
    std::cout << std::setw(2) << i << '\t' << std::string(h[i] / 10000, '*')
              << '\n';
  }
}
...
// use atomic to synchronize access to the hist vector
std::vector<int> hist_atomic(int N, int bins) {
  std::vector<int> hist(bins, 0);

  Timer t("Histogram calculation using atomic.");
#pragma omp parallel for
  for (int i = 0; i < N; ++i) {
    int bin = sample_from_normal_distribution();
    ++hist[bin];
...
#pragma omp single
    {
      ...
      hist_large.resize(thread_count * bins); // one section for each thread
    }
    int offset = omp_get_thread_num() * bins;
#pragma omp for
    for (int i = 0; i < N; ++i) {
      ++hist_large[offset + sample_from_normal_distribution()];
    }
...
#pragma omp parallel
  {
    ...
    while (1 > std::chrono::duration_cast<std::chrono::milliseconds>(
                   std::chrono::high_resolution_clock::now() - t_start_)
                       .count() /
                   1e3) {
    };
  ...
  // atomic hist computation
  auto atomic_hist = hist_atomic(N, bins);
  print_hist(atomic_hist);
  sum = std::accumulate(atomic_hist.begin(), atomic_hist.end(), 0);
  std::cout << "atomic hist (sum): " << sum << std::endl;
